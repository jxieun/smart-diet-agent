# 🐍 분석 엔진 API 및 메시지 명세 (Analysis Engine Specification)

## 📋 서비스 정보
| 항목 | 내용 |
| :--- | :--- |
| **기본 URL** | `http://localhost:8000` |
| **역할** | RabbitMQ 메시지 수신 및 메뉴판 AI 분석 (OCR) |
| **상태** | 개발 중 (F-06 완료) |

---

## 🐰 RabbitMQ 메시지 수신 규격 (Message Consumer)
NestJS로부터 발행된 분석 요청 메시지를 처리하는 규격입니다.

- **Exchange**: `menu_analysis_exchange`
- **Queue**: `menu_analysis_queue`
- **Routing Key**: `analyze.request`
- **Payload Structure (JSON)**:
  ```json
  {
    "taskId": "uuid-string",
    "imageUrl": "uploads/image-path.jpg",
    "userId": "uuid-string",
    "targetCalories": 2000 // 선택 사항
  }
  ```

## 🚦 내부 상태 확인 API (Health Check)
분석 엔진의 가동 상태를 확인하기 위한 엔드포인트입니다.

### 1. 서비스 상태 조회
- Endpoint: GET /

- Response (200 OK):

 ```JSON
{
  "status": "Analysis Engine is running!"
}
 ```

## ⚠️ 트러블슈팅 기록 (Troubleshooting)
- Python 환경 충돌: 최신 OS 환경에서 python 명령어 대신 `python3`를 명시하여 가상환경을 구축함.

- Zsh 패턴 매칭 오류: pip install fastapi[all] 실행 시 대괄호 해석 문제를 방지하기 위해 따옴표("")를 사용하여 패키지 설치 완료.

## 🐰 RabbitMQ 메시지 응답 규격 (Message Producer) ✨ New!
분석이 완료된 후 FastAPI가 NestJS로 돌려보내는 메시지 규격입니다.

- **Exchange**: `menu_analysis_exchange`
- **Routing Key**: `analyze.result`
- **Payload Structure (JSON)**:
  ```json
  {
    "taskId": "uuid-string",
    "status": "COMPLETED",
    "extractedText": [
      "비빔밥",
      "12,000",
      "떡볶이",
      "8,000"
    ]
  }
  ```

## 🛠️ 트러블슈팅 업데이트 (F-07)
- OCR 인식 실패 (Empty List): 이미지 내 텍스트가 너무 작거나 음식 사진만 있을 경우 빈 리스트([])가 반환되는 현상을 확인. 메뉴판 위주의 선명한 이미지를 권장하도록 설계함.

- 이미지 경로 참조 오류: `os.path.exists()`를 통해 실제 파일 존재 여부를 디버깅하여, `../smart-diet-backend/uploads/` 경로 접근 권한 및 위치 문제를 해결함.